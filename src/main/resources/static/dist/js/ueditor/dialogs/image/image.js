...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        setImage: function(img){
            ...
            if(src) {
                /* 设置表单内容 */
                $G("width").value = ""; //设置默认大小
                // $G("height").value = img.height || ''; //不设置高度
        ...
        setPreview: function(){
            ...
            //预览宽度设置
            if($G('width').value){
                width = ~$G('width').value.indexOf("%") ? $G('width').value : $G('width').value + "px";
            }else{
                width = "100%";
            }
            ...
            if(url) {
                preview.innerHTML = '<img src="' + url + '" width="' + width + '" border="' + border + 'px solid #000" title="' + title + '" />';
            }
        },
        getInsertList: function () {
            ...
            if(data['url']) {
                return [{
                    src: data['url'],
                    _src: data['url'],
                    width: data['width'] ? ~data['width'].indexOf("%") ? data['width'] : data['width'] + "px" : '',
                    // height: data['height'] || '',
                    border: data['border'] || '',
                    floatStyle: data['align'] || '',
                    vspace: data['vhSpace'] || '',
                    title: "onlineImg" || '', //标识是网络在线图片
                    alt: data['title'] || ''
